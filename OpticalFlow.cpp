...
OpticalFlow::OpticalFlow(int sizeX, int sizeY)
{
    ...
    // Establezco las dimensiones de las imágenes a analizar
    XDim = sizeX;
    YDim = sizeY;
    ...
    // Inicializo el numero de vectores y de puntos de análisis
    numVectores = 0;
    numPuntosAnalisis = 0;
...
void OpticalFlow::alturaMaxima(TMatriz *matriz1, int &h1, int &h2, int &x1, int &y1, int &x2, int &y2){
  ...
  for(int j=0;j<matriz1->getDimY();j++)
        ...
        // 1º Aprox : Si no terminan en la misma columnas y estamos cometiendo un error!!!
